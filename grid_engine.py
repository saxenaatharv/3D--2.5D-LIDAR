...
class VariableResolutionGrid:
    ...
    # ---- read-only point -> cell lookup, used by eval_accuracy.py --------
    # Purely additive: does not create cells, does not touch insert_points'
    # behaviour, and is not called anywhere in the existing pipeline. Safe
    # to add without changing run_inference.py / finetune.py outputs.
    def locate(self, x_world, y_world):
        """
        Given a single WORLD-frame (x, y), return the (ring_id, ix, iy) key
        this grid would file it under -- the same ring/floor formula used by
        insert_points, but without ever creating or touching a cell. Returns
        None if the point falls outside every ring (r >= max radius, or no
        ring covers that radius). z is intentionally not needed: cell keys
        in this 2.5D grid are purely a function of (x, y).
        """
        x = x_world - self.ego_x
        y = y_world - self.ego_y
        r = float(np.hypot(x, y))
        if r >= self._max_r:
            return None
        ring_id, ring = self._find_ring(r)
        if ring is None:
            return None
        cs = ring["cell_size"]
    ...
    def cell_at(self, x_world, y_world):
        """Convenience: CellStats at (x_world, y_world), or None if unobserved."""
        key = self.locate(x_world, y_world)
        if key is None:
            return None
